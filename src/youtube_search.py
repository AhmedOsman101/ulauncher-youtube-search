...
class YoutubeSearch:
  ...
  def execute(self):
    response = requests.get(
        SEARCH_BASE_URL + urllib.parse.quote(self.query)).text
    results = []
          ...
          results.append(
              {
                  ...
                  .get("runs", [{}])[0]
                  .get("text", None),
                  "date": video_data.get("publishedTimeText", {}).get(
                      "simpleText"
                  ),
                  "duration": video_data.get("lengthText", {}).get(
                      "simpleText", 0
                  ),
                  "views": video_data.get("viewCountText", {}).get(
                      "simpleText", 0
                  ),
                  "url": BASE_URL
